...
- [x] **T-300 项目归档功能 (归档 / 恢复)** [P1·老板需求单] (`aa74746`)
...
- [x] **T-302 流水删除 — UI (详情页删除按钮 + 确认 + 原因)** [P1·老板需求单·依赖 T-301] (`d72f6bc`)
...
- [x] **T-308 删除冲销过的流水（配对一起删）** [P1·老板实测] — 2026-05-16 完成
...
- [x] **T-310 合伙人看不到流水被驳回的原因** [P1·老板实测·体验红线] — 80b391f
  **dengche 验收**: 24 e2e 全绿(含新增 rejected-reason.spec) + 真实浏览器以合伙人登录实测——/me 历史流水被驳回行红字显示原因、行可点进详情、详情页顶部红卡明示「已驳回·原因」。
...
- [x] **T-312 合伙人流水被驳回的站内提醒** [P1·老板实测·体验红线] — 2026-05-19 完成
  **dengche 验收**: 导航"我的回报"红点徽章 + /me 顶部红色横幅，数字=本人 REJECTED 流水数，均按角色卡(仅合伙人，OWNER 不显示)。全套 56 e2e 双引擎全绿；tsc + build 绿。验收中修了 codex 2 处：spec strict-mode 冲突(多条共用驳回原因没加 .first())、/me 横幅漏角色门(OWNER 有被驳回流水时会误显示)。
  **背景**: 老板实测——驳回合伙人(田锦涛)的流水后, 合伙人说"没接到驳回信息"。dengche 已查实: T-310 在线上正常, 合伙人 /me 页确实显示了全部 5 条驳回原因——但账本**不主动提醒**, 合伙人不去翻"我的回报"页就不知道有流水被驳回。T-310 当初按部门边界"不做通知推送"只做了"打开页面能看见", 缺"提醒去看"这一环。本工单补站内提醒(**仍不做外部消息推送**, 短信/微信/邮件不碰——只做账本界面内的视觉信号)。
  **改哪 + 怎么改**:
  1. 顶部导航"我的回报"链接旁加红点徽章, 数字 = 当前登录合伙人名下 `status=REJECTED` 的流水数(仅本人, `createdById = 当前用户`)。导航在 app layout(server component), 加一个轻量 count 查询。count=0 不显示徽章。
  2. `/me` 页顶部: 若该合伙人有 REJECTED 流水, 加一个醒目横幅"你有 N 笔流水被驳回, 见下方历史流水, 请按驳回原因修正后重新提交"。
  **不做**: 外部消息推送(部门边界不变); 不引入"已读"状态(不动 schema)——有 REJECTED 就一直提醒, 合伙人改正重交后该条不再 REJECTED, 提醒自然消失。
  **约束**: 不碰 prisma schema / 不装新依赖 / 不 push。
  **验收**: 合伙人登录后, 有被驳回流水时导航徽章 + /me 横幅都显示且数字正确; 无被驳回流水时不显示; OWNER 不受影响; e2e 补用例(Chromium + WebKit 双引擎); 全套 e2e 全绿; `npx tsc --noEmit` + `pnpm exec next build --webpack` 绿。
